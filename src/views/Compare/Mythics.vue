  ...
  <div>
    <div class="compViews__playerOne">
      ...
      <div class="playerOne__mythics" v-if="oneStats">
        ...
        <div class="player_roles" v-if="oneScores">
        ...
        <ul>
          <li class="mythic-dungeon-item" v-for="(run, index) in getRunsOne" :key="index">
            <div class="timestamps">
              <p class="dungeon-timestamp">{{completedAt(run.completed_timestamp)}}</p>
              <p
                :class="{withinTimeRun: run.is_completed_within_time, failedRun: !run.is_completed_within_time}"
              >{{durationTime(run.duration)}}</p>
            </div>
            <div class="dungeon-item-title">
              <span v-for="(affix, j) in run.keystone_affixes" :key="j">
                <a class="affix-name" :href="'https://wowhead.com/affix=' + affix.id">
                  ...
                  <div v-else-if="affix.id === 119">
                    <img class="affix-image" :src="affixIcons[18].icon" />
                  </div>
                  <div v-else>
            ...
            <div class="dungeon-members">
              <ul class="members-list">
                <li v-for="(m, index) in run.members" :key="index">
                  <p class="member-name" :class="{sameMember: m.character.name === oneStats.name}">
                    {{m.character.name}}
                    <span class="character-realm">- {{m.character.realm.slug}}</span>
                  </p>
                  <p>
                    iLvL:
                    <span class="member-itemlevel">{{m.equipped_item_level}}</span>
                  </p>
                </li>
              </ul>
            </div>
    ...
    <div class="compViews__playerOne">
      ...
      <div class="playerTwo__mythics" v-if="twoStats">
        ...
        <div class="player_roles" v-if="twoScores">
          <div class="player_roles_header">
        ...
        <ul>
          <li class="mythic-dungeon-item" v-for="(run, index) in getRunsTwo" :key="index">
            <div class="timestamps">
              <p class="dungeon-timestamp">{{completedAt(run.completed_timestamp)}}</p>
              <p
                :class="{withinTimeRun: run.is_completed_within_time, failedRun: !run.is_completed_within_time}"
            ...
            <div class="dungeon-members">
              <ul class="members-list">
                <li v-for="(m, index) in run.members" :key="index">
                  <p class="member-name" :class="{sameMember: m.character.name === twoStats.name}">
                    {{m.character.name}}
                    <span class="character-realm">- {{m.character.realm.slug}}</span>
                  </p>
                  <p>
...
import Beguiling from "@/assets/mythics/119_affix_icon.jpg";
...
export default {
  ...
  methods: {
    durationTime(val) {
      ...
      return `${
        hour.getUTCHours() < 10 ? "00" : ""
      }:${minute.getMinutes()}:${seconds.getSeconds()}`;
    },
