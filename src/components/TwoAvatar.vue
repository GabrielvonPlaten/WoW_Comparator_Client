<template>
  <div class="playerTwo_avatar avatar" v-if="twoInfo">
    <img
      ...
      @error="changeAvatarUrl(twoInfo.race, twoInfo.gender)"
    />
    <h2>{{twoInfo.name}}</h2>
    <h2>{{twoInfo.guild}}</h2>
    <h2>
      {{twoInfo.level}} -
      <span
        :style="{color: classColor[twoInfo.class]}"
      >{{classes[twoInfo.class]}}</span>
    </h2>
...
export default {
  ...
  computed: {
    twoInfo() {
      ...
      const d = store.state.playerTwoStats.lastModified; // Unix Timestamp
